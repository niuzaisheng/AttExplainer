# Model interpretability results analysis process
import sys
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run model interpretability analysis process")
    ...
    parser.add_argument("--discribe", type=str, default="Model interpretability evaluation process")
...
def get_rewards(seq_length, original_prob, post_acc, post_prob, game_status, game_step):
    ...
    post_rewards = delta_p + 10 * ifdone - game_step / config.max_game_steps

    return delta_p, post_rewards, ifdone, musked_token_num, unmusk_token_num, musked_token_rate, unmusked_token_rate
...
def one_step(transformer_model, original_pred_labels, post_batch, seq_length, bins_num, lm_device, dqn_device, use_random_matrix=False):
    ...
    return all_attentions, post_acc, post_prob, post_pred_labels
...
for simulate_step, simulate_batch in enumerate(eval_dataloader):
    ...
    post_batch, actions, last_game_status, action_value = dqn.initial_action(simulate_batch, special_tokens_mask, seq_length, batch_max_seq_length, dqn.device)
    all_attentions, post_acc, post_prob, post_pred_labels = one_step(transformer_model, original_pred_labels, simulate_batch, seq_length, config.bins_num,
    ...
    for game_step in range(epoch_game_steps):
        ...
        post_batch, actions, now_game_status, action_value = dqn.choose_action_for_eval(simulate_batch, seq_length, special_tokens_mask,
                                                                                        all_attentions, last_game_status)
        next_attentions, post_acc, post_prob, post_pred_labels = one_step(transformer_model, original_pred_labels, post_batch, seq_length, config.bins_num,
                                                                          lm_device=lm_device, dqn_device=dqn.device, use_random_matrix=config.use_random_matrix)

        delta_p, rewards, ifdone, musked_token_num, unmusk_token_num, musked_token_rate, unmusked_token_rate =\
            get_rewards(seq_length, original_prob, post_acc, post_prob, now_game_status, game_step)
...
reslut["cumulative_rewards"] = np.mean(all_cumulative_rewards)
