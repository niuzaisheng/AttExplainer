...
import numpy as np
try:
    from tsnecuda import TSNE # for more quick t-sne
except:
...
base_debug_config = {
    ...
    "bins_num": 32,
    "data_set_name": "emotion",
    "max_game_steps": 100,
    "done_threshold": 0.8,
    "use_categorical_policy": False,
    "gpu_index": 0,
    "is_agent_on_GPU": False,
    "seed": 42
}

# Some case study reported in paper
# Example 1 : Explain-gradient-mask
debug_config = {
    "task_type": "explain",
    "features_type": "gradient",
    "token_replacement_strategy": "mask",
    "dqn_weights_path": "saved_weights/emotion_2022-12-12-09-18-41/dqn-200000.bin",
    **base_debug_config
}

# Example 2 for random feature
# debug_config = {
#     "task_type":"explain",
#     "features_type":"random",
#     "token_replacement_strategy":"mask",
#     "dqn_weights_path":"saved_weights/emotion_2022-12-06-10-40-34/dqn-200000.bin",
#     **base_debug_config
# }

# Example 3 for mixture feature
# debug_config = {
#     "task_type": "explain",
#     "features_type": "mixture",
#     "token_replacement_strategy": "mask",
#     "dqn_weights_path": "saved_weights/emotion_2023-05-05-04-14-33/dqn-200000.bin",
#     **base_debug_config
# }

# convert debug_config to argparse.Namespace
config = argparse.Namespace(**debug_config)
config_base64 = base64.b64encode(str(config).encode("utf-8")).decode("utf-8")[0:10]

input_text = config.input_text
input_text_base64 = base64.b64encode(input_text.encode("utf-8")).decode("utf-8")[0:10]
...
transformer_model, _, _ = get_dataloader_and_model(
    config, dataset_config, tokenizer, return_simulate_dataloader=False, return_eval_dataloader=False)
...
def get_rewards(original_seq_length=None,
    ...
    original_seq_length = (torch.FloatTensor(
        original_seq_length) - token_quantity_correction)
    original_seq_length, game_status = keep_tensor_in_same_device(
        original_seq_length, game_status, device="cpu")
    ...
    if config.task_type == "attack":
        ...
        delta_prob, if_success = keep_tensor_in_same_device(
            delta_prob, if_success, device="cpu")
    ...
    if features_type == "gradient":
        ...
        with torch.no_grad():
            ...
            if features_type == "statistical_bin":
                extracted_features = get_attention_features(
                    post_outputs, post_batch["attention_mask"], seq_length, config.bins_num)
            elif features_type == "const":
                extracted_features = get_const_attention_features(
                    post_outputs, config.bins_num)
            elif features_type == "random":
                extracted_features = get_random_attention_features(
                    post_outputs, config.bins_num)
            elif features_type == "effective_information":
                extracted_features = get_EI_attention_features(
                    post_outputs, seq_length)
...
dqn = DQN(config, do_eval=True, mask_token_id=MASK_TOKEN_ID,
...
actions, now_game_status = dqn.initial_action(
    batch, special_tokens_mask, seq_length, batch_max_seq_length, dqn.device)
...
for game_step in range(epoch_game_steps):
    ...
    post_batch, actions, next_game_status, next_special_tokens_mask = dqn.choose_action(
        original_batch, seq_length, special_tokens_mask, now_features, now_game_status)
    ...
    print(f"post_batch is {post_batch}")
...
np.savez(f"saved_results/explain_path_{config_base64}.npz",
         config=config,
...
# 1. read explain_path
data = np.load(f"saved_results/explain_path_{config_base64}.npz", allow_pickle=True)
...
# %% Step 6: Plot path cluster perfer situation
...
plt.imshow(cluster_mask_counter[:, 1:-1], cmap=cmap, interpolation='nearest')
...
# no ticks in colorbar
cb = plt.colorbar(orientation='horizontal', shrink=0.7, aspect=20)
cb.set_ticks([])
...
# %% Step 7: Plot the Perturbtion Space and the Explain Path
plt.figure(figsize=(5, 5))
colors = ["edf2fb", "e2eafc", "d7e3fc", "ccdbfd", "c1d3fe", "b6ccfe", "abc4ff"]
colors = [f"#{i}" for i in colors]
cmap = mcolors.LinearSegmentedColormap.from_list("n", colors)

# sample_label_probs = all_label_probs[random_sample_index]
# color adjust by label_prob, range from 0 to 1
plt.scatter(all_output_reprs_tsne[:, 0], all_output_reprs_tsne[:, 1], c=all_label_probs, cmap=cmap, s=3, vmin=0, vmax=1)

# colorbar ticks from 0 to 1
cb = plt.colorbar(orientation='horizontal', label='probability')
...
plt.scatter(explain_path_tsne[0, 0], explain_path_tsne[0, 1], c="r", marker="o", s=100)
# end point of explain path
plt.scatter(explain_path_tsne[-1, 0], explain_path_tsne[-1, 1], c="r", marker="*", s=100)
...
# plt.title("Perturbation Examples Space")
plt.tight_layout()
plt.savefig(f"saved_results/perturb_space_{config_base64}.pdf")
plt.show()


# %%
# convert explain_path to token list
...
# for i in range(len(sample_explain_path_index)):
#     sample_explain_path_nearest_cluster_label.append(
#         cluster_labels[sample_explain_path_index[i]])
...
# with open(f"logs/{exp_name}_explain_path_table.tex", "w") as f:
#     f.write(latex)
